 ...
 * AlphaPortal refresh token exists.
 */
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('node', [entry], {
      ...
      env: {
        ...
        // Point the session store at a throwaway path so a developer's real
        // saved token never leaks into (or is mutated by) the boot test.
        ALPHAPORTAL_SESSION_FILE: join(cwd, 'session.json'),
      },
...
describe('server boot (built artifacts)', () => {
  it('bundled .mcpb (dist/bundle.js) boots WITHOUT node_modules and lists its tools', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'alphaportal-mcpb-'));
    try {
      ...
      expect(tools).toContain('alphaportal_session_status');
      expect(tools).toContain('alphaportal_list_students');
    } finally {
